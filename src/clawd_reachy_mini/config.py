...
CONFIG_SEARCH_PATHS = [
    Path("clawd.yaml"),  # current directory
    Path("clawd.yml"),
    Path.home() / ".clawd-reachy-mini" / "config.yaml",
    Path.home() / ".clawd-reachy-mini" / "config.yml",
]
...
def _find_config_file(explicit_path: str | Path | None = None) -> Path | None:
    ...
    env_path = os.environ.get("CLAWD_CONFIG")
    if env_path:
        p = Path(env_path)
        return p if p.is_file() else None
...
def load_config(config_path: str | Path | None = None) -> Config:
    ...
    Args:
        config_path: Explicit path to a YAML config file. If None,
            searches CLAWD_CONFIG env var, then default locations.
    """
    config = Config()
